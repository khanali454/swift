 ...
 */
function get_freelancer_active_projects(FreelancerIdentity $provider, $page = 1, $limit = 10)
{
    if (!$provider->accessToken->hasExpired()) {
        $offset = isset($page) && is_numeric($page) ? (intval($page) - 1) * $limit : 0;
        $request = $provider->getAuthenticatedRequest(
            'GET',
            $provider->apiBaseUri . "/projects/0.1/projects/active?limit=$limit&offset=$offset&full_description=true&job_details=true&local_details=true&location_details=true&upgrade_details=true&user_country_details=true&user_details=true&user_employer_reputation=true&user_status=true&sort_field=submitdate&compact=true&new_errors=true&new_pools=true",
        ...
        throw new FreelancerIdentityException();
    }
}

function get_freelancer_project(FreelancerIdentity $provider, int $project_id)
{
    $request = $provider->getAuthenticatedRequest(
        'GET',
        $provider->apiBaseUri . '/projects/0.1/projects/' . $project_id . '?full_description=true&job_details=true&local_details=true&location_details=true&upgrade_details=true&user_country_details=true&user_details=true&user_employer_reputation=true&user_status=true&sort_field=submitdate&compact=true&new_errors=true&new_pools=true",',
        $provider->accessToken->getToken(),
        [
            "headers" => ["Content-Type" => "application/json"],
        ]
    );
    $project_response = $provider->getParsedResponse($request);
    if (is_array($project_response) && isset($project_response['result'])) {
 ...
 * @throws Exception
 */
function bid_freelancer_project(FreelancerIdentity $provider, int $project_id, int $amount, int $period, string $description, int $milestone_percentage = 100): void
{
    if (!$provider->accessToken->hasExpired()) {
        $project = get_freelancer_project($provider, $project_id);
        ...
        if ($project) {
            ...
            $user_id = get_freelancer_self($provider)['id'];
            /**
             * bidding now!
             */
            $request = $provider->getAuthenticatedRequest(
                'POST',
                $provider->apiBaseUri . '/projects/0.1/bids/',
                $provider->accessToken->getToken(),
                [
                    "headers" => ["Content-Type" => "application/json"],
                    "body" => json_encode([
            ...
            $bid_response = $provider->getParsedResponse($request);
        } else {
 ...
 * @throws Exception
 */
function get_content_from_chat_gpt($prompt)
{
    global $config;
    $token = $config['chat_gpt']['bearer_token'];
    if (!$token) {
        throw new Exception('chat gpt bearer token not found');
    }
    $data = [
        "model" => "gpt-3.5-turbo",
        "max_tokens" => 2048,
    ...
    $result = json_decode($result, true)['choices'][0]['message']['content'];
    curl_close($ch);
    return $result;
}

function get_freelancer_self(FreelancerIdentity $provider)
{
    /**
     * Get current user
     */
    $request = $provider->getAuthenticatedRequest(
        'GET',
        $provider->apiBaseUri . '/users/0.1/self/',
        $provider->accessToken->getToken(),
        [
            "headers" => ["Content-Type" => "application/json"],
        ]
    );
    $user_response = $provider->getParsedResponse($request);
    if (!is_array($user_response) || !isset($user_response['result'])) {
        throw new FreelancerIdentityException();
    }
 ...
 * @return bool
 */
function add_user_freelancer_bid(int $user_id, array $project): bool
{
    ...
    $sql = "Insert into " . $config['db']['pre'] . "bid_entries (user_id, project) values (:user_id, :project)";
...
function list_freelancer_bids($user_id): array
{

    global $config;
    return ORM::for_table($config['db']['pre'] . 'bid_entries')
        ->where('user_id', $user_id)->find_array();
}

function get_freelancer_bid_status(FreelancerIdentity $provider, $project, $user_id)
{
//    $user_id = 25955717;
    $request = $provider->getAuthenticatedRequest(
        'GET',
        $provider->apiBaseUri . "/projects/0.1/projects/$project/bids/",//?projects=$project&bidders=$user",
        $provider->accessToken->getToken(),
        [
            "headers" => ["Content-Type" => "application/json"],
        ]
...
{
    include ROOTPATH . '/includes/lib/php-markdown/Michelf/Markdown.inc.php';
    $ai_message = Michelf\Markdown::defaultTransform($ai_message);
